...
if Key_ins7.keys in ['', [], None]:  # No response was made
    Key_ins7.keys = None
thisExp.addData('Key_ins7.keys',Key_ins7.keys)
if Key_ins7.keys != None:  # we had a response
    thisExp.addData('Key_ins7.rt', Key_ins7.rt)
    thisExp.addData('Key_ins7.duration', Key_ins7.duration)
thisExp.nextEntry()
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        minutes = str(int(time_left / 60)).zfill(2)
        seconds = str(int(time_left % 60)).zfill(2)
    ...
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        if Image_ins8.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Image_ins8.tStartRefresh + 1-frameTolerance:
                # keep track of stop time/frame for later
