...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/xwx/Desktop/lab/old/semantic_fluency_fulla-master/semantic_fluency_fullA.py',
...
Image_ins9 = visual.ImageStim(
    win=win,
    name='Image_ins9', 
    image='resources/ins9.png', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(1.2, 0.8),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
Key_ins9 = keyboard.Keyboard()
...
# update component parameters for each repeat
# keep track of which components have finished
Cat_practiceRegisComponents = [Text_Catpracticeregis, Mic_catpractice]
...
while continueRoutine and routineTimer.getTime() < 120.0:
    ...
    # if Text_Catpracticeregis is stopping this frame...
    if Text_Catpracticeregis.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > Text_Catpracticeregis.tStartRefresh + 120-frameTolerance:
            # keep track of stop time/frame for later
            Text_Catpracticeregis.tStop = t  # not accounting for scr refresh
            Text_Catpracticeregis.frameNStop = frameN  # exact frame index
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Text_Catpracticeregis.stopped')
            # update status
            Text_Catpracticeregis.status = FINISHED
            Text_Catpracticeregis.setAutoDraw(False)
    ...
    if Mic_catpractice.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > Mic_catpractice.tStartRefresh + 120-frameTolerance:
            # keep track of stop time/frame for later
...
for thisTrial in trials:
    ...
    # keep track of which components have finished
    Cat_testregisComponents = [Text_cattextregis, Mic_cattestresgis]
    ...
    while continueRoutine and routineTimer.getTime() < 120.0:
        ...
        # if Text_cattextregis is stopping this frame...
        if Text_cattextregis.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Text_cattextregis.tStartRefresh + 120-frameTolerance:
                # keep track of stop time/frame for later
                Text_cattextregis.tStop = t  # not accounting for scr refresh
                Text_cattextregis.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Text_cattextregis.stopped')
                # update status
                Text_cattextregis.status = FINISHED
                Text_cattextregis.setAutoDraw(False)
        ...
        if Mic_cattestresgis.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Mic_cattestresgis.tStartRefresh + 120-frameTolerance:
                # keep track of stop time/frame for later
    ...
    _Key_ins9_allKeys = []
    # keep track of which components have finished
    ins9Components = [Image_ins9, Key_ins9]
    ...
    while continueRoutine:
        ...
        # if Image_ins9 is starting this frame...
        if Image_ins9.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            Image_ins9.frameNStart = frameN  # exact frame index
            Image_ins9.tStart = t  # local t and not account for scr refresh
            Image_ins9.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Image_ins9, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Image_ins9.started')
            # update status
            Image_ins9.status = STARTED
            Image_ins9.setAutoDraw(True)
        
        # if Image_ins9 is active this frame...
        if Image_ins9.status == STARTED:
            # update params
            pass
        ...
        # if Key_ins9 is starting this frame...
        if Key_ins9.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            Key_ins9.frameNStart = frameN  # exact frame index
            Key_ins9.tStart = t  # local t and not account for scr refresh
            Key_ins9.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Key_ins9, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Key_ins9.started')
            # update status
            Key_ins9.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(Key_ins9.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(Key_ins9.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if Key_ins9.status == STARTED and not waitOnFlip:
            theseKeys = Key_ins9.getKeys(keyList=['space'], waitRelease=False)
            _Key_ins9_allKeys.extend(theseKeys)
            if len(_Key_ins9_allKeys):
                Key_ins9.keys = _Key_ins9_allKeys[-1].name  # just the last key pressed
                Key_ins9.rt = _Key_ins9_allKeys[-1].rt
                Key_ins9.duration = _Key_ins9_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    for thisComponent in ins9Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # check responses
    if Key_ins9.keys in ['', [], None]:  # No response was made
        Key_ins9.keys = None
    trials.addData('Key_ins9.keys',Key_ins9.keys)
    if Key_ins9.keys != None:  # we had a response
        trials.addData('Key_ins9.rt', Key_ins9.rt)
        trials.addData('Key_ins9.duration', Key_ins9.duration)
